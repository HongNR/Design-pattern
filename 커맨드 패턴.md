...
2. **비동기 작업 큐 및 대기열 (Spring `@Async`, Message Queue / Kafka, RabbitMQ)**
   - 대용량 이메일/알림톡 발송, PDF 리포트 생성, 외부 PG 결제 승인 요청을 `Command` 객체(DTO 직렬화)로 큐에 적재 후, 백그라운드 Worker 프로세스가 순차적으로 꺼내어 실행 (`command.execute()`).
3. **CQRS 아키텍처 (Command Query Responsibility Segregation)**
...
## 8. 🔍 심화: Spring Request DTO도 커맨드 패턴일까?

> **"네, 아키텍처 관점에서 Spring의 Request DTO는 커맨드 패턴의 철학을 반영한 고도화된 변형입니다."**

Spring MVC 공식 문서에서도 HTML 폼 및 HTTP 요청 파라미터를 바인딩하는 DTO/객체를 공식 용어로 **"Command Object (커맨드 객체)"** 또는 **"Form Backing Object"**라고 명시하고 있습니다.

### 🤝 커맨드 패턴과 Spring Request/Response 매핑
...
### ⚖️ 클래식 GoF 패턴 vs Modern Spring / CQRS 아키텍처
